...
<t:pageTemplate pageTitle="Edit Car">
    <h1>Edit Car</h1>
    <form class="row g-3 needs-validation" method="POST" action="${pageContext.request.contextPath}/EditCar" novalidate>
        <div class="col-md-6">
            <label for="licensePlate" class="form-label">License plate</label>
            <input type="text" class="form-control" id="licensePlate" name="license_plate" value="${car.licensePlate}" required>
            <div class="invalid-feedback">Please provide a license plate.</div>
        </div>

        <div class="col-md-6">
            <label for="parkingSpot" class="form-label">Parking spot</label>
            <input type="text" class="form-control" id="parkingSpot" name="parking_spot" value="${car.parkingSpot}" required>
        ...
        <div class="col-12">
            ...
            <select class="form-select" id="ownerId" name="owner_id" required>
                ...
                <c:forEach var="owner" items="${users}">
                    <option value="${owner.id}" ${car.ownerName eq owner.username ? 'selected' : ''}>${owner.username}</option>
                </c:forEach>
            </select>
